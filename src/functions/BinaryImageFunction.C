...
BinaryImageFunction::BinaryImageFunction(const std::string & name, InputParameters parameters) :
    ...
    _image_mesh_function(NULL)
{
  ...
  _serialized_solution->init(image_sys.n_dofs(), false, SERIAL);
  readImage();


  _image_mesh_function = new MeshFunction(*_image_es_ptr, *_serialized_solution, image_sys.get_dof_map(), 0);

}

BinaryImageFunction::~BinaryImageFunction()
{
...
Real
BinaryImageFunction::value(Real t, const Point & p)
{
//  return 1.0;
...
//std::cout << "pixel(" << x << ", " << y <<  ") = " << pixel[0] << " " << pixel[1] << " " << pixel[2] << std::endl;
        ...
        _serialized_solution->set(idx, value);
        idx++;
